...
console.log = function (d, ignoresave = false, DBdebugLog = false) {
  if (!ignoresave) {
    if (DBdebugLog) db_log_file.write(util.format(d) + '\n');
    else log_file.write(util.format(d) + '\n');
  }
...
console.error = function (d, supress = false) {
  ...
  log_file.write('---------------------------------\n');
  if (!uignoreErrors) log_stdout.write(util.format(d) + '\n');
};
...
function botOn(bot) {
  ...
  bot.on('ready', function (evt) {
    ...
    if (fridayData.friday === undefined) global.fridayCounter = {};
    else global.fridayCounter = fridayData;
  ...
  for (const file of commandFiles) {
    //adds all commands in the commands folder
  ...
  bot.on('guildScheduledEventUpdate', async (eold, enew) => {
    EventDB(enew, 'update');
  });
  bot.on('guildScheduledEventDelete', async (event) => {
    EventDB(event, 'delete');
  });

  // v14 works
  bot.on('guildScheduledEventUserAdd', async (event, user) => {
    EventDB(event, 'useradd', user);
  });
  bot.on('guildScheduledEventUserRemove', async (event, user) => {
    EventDB(event, 'userremove', user);
  });
...
var cleanupFn = function cleanup() {
  DMMePlease(
    'https://media.discordapp.net/attachments/979881683790733333/1344098978438053969/Bear_hugs__eskimo_kisses..png?ex=67bfad38&is=67be5bb8&hm=6825f9faae0e9e3e300c7f0e2cc1e6b320f8076449e21d1d31512814f236b3f6&=&format=webp&quality=lossless&width=1400&height=993'
  );
