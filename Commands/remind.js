...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option.setName('message').setDescription('The message to me reminded of!').setRequired(true)
    )
    .addStringOption((option) =>
      option.setName('time').setDescription('The time to be reminded at!').setRequired(true)
    )
    .addStringOption((option) =>
      option
        .setName('date')
        .setDescription('The date of the reminder, optional defaults within next 24 hours!')
    ),
  /**
  ...
  async execute(interaction, bot) {
    ...
    await interaction.editReply({
      content:
        ...
        ':R> which is <t:' +
        milisec +
        ':F>, if baba crashes, tough luck no reminder!',
      ephemeral: true,
    });
  },
};
