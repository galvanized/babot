...
} = require('../Functions/HelperFunctions/slashFridayHelpers');
...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt.setName('text').setDescription('The text for morshu to transcribe.').setRequired(true)
    )
    ...
    .addStringOption((option) =>
      option
        ...
        .addChoices({ name: 'Audio', value: 'audio' }, { name: 'Video', value: 'video' })
    ),
  /**
  ...
  async execute(interaction, bot) {
    ...
    for (var i = 0; i < textSplit.length; i++) {
      ...
      if (morshuFile.file != null) filesOfMorsh.push(morshuFile.file);
    }

    if (filesOfMorsh.length == 0)
      await interaction.editReply(
        "Morshu couldn't transcribe the text, it may have been too long or there was an error!"
      );
    else {
      var objectSend = {
      ...
      if (personaltext != null && personaltext != '') objectSend.content += '\n' + personaltext;

      if (subtitles) objectSend.content += '\n```' + text + '```';
