...
const {
  ...
  handleButtonsEmbed,
  preformEasterEggs,
} = require('../Functions/HelperFunctions/basicHelpers.js');
...
const {
  ...
  movetoChannel,
  timedOutFrog,
} = require('../Functions/HelperFunctions/adminHelpers.js');
...
async function babaMessage(bot, message) {
  ...
  if (
    msgContent.includes(yr - 1) &&
    msgContent.includes('560231259842805770') &&
    msgContent.includes('563063109422415872') &&
    !message.author.bot &&
    message.author.id == '360228104997961740'
  ) //if message contains baba and is not from bot
  {
    let rawdata = fs.readFileSync(__dirname.replace('TextCommands', '') + '/babotdata.json');
    let baadata = JSON.parse(rawdata);
  ...
  if (msgContent.includes('!baba')) //if message contains baba and is not from bot
  {
    ...
    if (msgContent.includes('baba is help') && message.author.bot) return;
    ...
    if (msgContent.includes('goodberries') || msgContent.includes('goodberry')) {
      ...
      babaGoodberrys(function (val) {
        ...
        for (var i = 0; i < evnts.length; i++) {
          ...
          resp +=
            evnts[i].summary + ' on ' + evnts[i].start.toLocaleDateString('en-US', options) + '\n';
        }

        if (resp == '') resp = 'No events found';
    ...
    if (
      msgContent.includes('flag') &&
      (msgContent.includes('night shift') || msgContent.includes('vibe time'))
    ) {
      message.channel.sendTyping();
      ...
      message.channel.send(flagcontent).catch((error) => {
        var newAttch = new Discord.AttachmentBuilder(getErrorFlag(), {
          name: 'errrrrr.png',
          description: 'No flag for you bud hee!',
...
*/
    ...
    if (
      msgContent.includes('wednesday') ||
      msgContent.includes('days until') ||
      msgContent.includes('when is') ||
      msgContent.includes('day of week')
    ) {
      message.channel.sendTyping();
      if (msgContent.includes('days until next wednesday')) message.channel.send(babaDayNextWed());
      ...
      for (var i = 0; i < texts.length; i++) {
        if (texts[i].files == null) {
          ...
          if (text == 'FUNNYDOW') await functionPostFunnyDOW('message', message, 3);
          else await message.channel.send(text);
        } else timedOutFrog(i, texts, message, templocal);
      }
    }
  }
  if (msgContent.includes('!bdelete')) //code to del and move to log
  {
    ...
    if (
      ...
      message.member.roles.cache.has(babadata.adminId)
    ) //check if admin
    {
  ...
  if (msgContent.includes('!bsetstatus')) //code to set game
  {
    ...
    ) //check if admin
    {
      var text = msgContent;
      var tyepe = -1;
      if (text.includes('idle')) tyepe = 'idle';
      if (text.includes('afk')) tyepe = 'idle';
      else if (text.includes('online')) tyepe = 'online';
      else if (text.includes('woke')) tyepe = 'online';
      else if (text.includes('invisible')) tyepe = 'invisible';
      else if (text.includes('offline')) tyepe = 'invisible';
      else if (text.includes('dnd')) tyepe = 'dnd';
      else if (text.includes('do not disturb')) tyepe = 'dnd';

      if (tyepe == -1) tyepe = 'online';
  ...
  if (msgContent.includes('!bsetgame')) //code to set game
  {
    ...
    ) //check if admin
    {
      ...
      var tyepe = -1;
      var lc = 2;
      if (text.includes('watching')) tyepe = 3;
      else if (text.includes('playing')) tyepe = 0;
      else if (text.includes('listening')) tyepe = 2;
      else if (text.includes('competing')) tyepe = 5;
      else if (text.includes('streaming')) tyepe = 1;
      ...
      var help = { type: tyepe };
      if (tyepe == 1) help.url = 'https://www.twitch.tv/directory/game/Baba%20is%20You';
