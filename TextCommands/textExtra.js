...
const {
  ...
  clearVCCList,
  DMMePlease,
} = require('../Functions/Database/databaseVoiceController.js');
  ...
  for (var key in obj) {
    if (obj.hasOwnProperty(key)) {
      if (typeof obj[key] === 'object' && obj[key] !== null) {
        ...
        if (!isNaN(key)) strg += obj[key];
        else strg += key + ': ' + obj[key];
    ...
    if (neww === undefined || old[key] != neww[key] || arraymode) {
      if (typeof old[key] === 'object' && old[key] !== null) {
        ...
        if (Array.isArray(old[key]) && Array.isArray(neww[key])) {
          if (old[key].length == neww[key].length && old[key].length == 0) continue;
        }
        var strg = '';
        if (isNaN(key)) strg += generateTabs(ind);
        var nind = ind;
        ...
        var strg = '';
        strg += generateTabs(ind);
        if (!isNaN(key)) strg += old[key] + ' -> ' + neww[key];
        else
...
function parseItems(old, neww) {
  if (old == undefined) {
    ...
    if (typeof old === 'object' && old !== null) {
      ...
      strg = twoObjectParseCompare(
        old,
        neww,
        1 - (am ? (neww !== null ? (old.length > 1 || neww.length > 1 ? 0 : 1) : 0) : 0),
        am
      );
    }
  }
  ...
  if (sentvalid) // put in own file or something eventually
  {
    ...
    if (msgContent.includes('🐸 debug')) //0 null, 1 spook, 2 thanks, 3 crimbo, 4 defeat
    {
      if (msgContent.includes('---')) {
        ...
        var rename = '';
        if (msgContent.includes('-n')) rename = '-n';
        else rename = '';

        if (msgContent.includes('0')) SetHolidayChan(g, 'null');
        else if (msgContent.includes('1')) SetHolidayChan(g, 'spook' + rename);
        else if (msgContent.includes('2')) SetHolidayChan(g, 'thanks' + rename);
        else if (msgContent.includes('3')) SetHolidayChan(g, 'crimbo' + rename);
        else if (msgContent.includes('4')) SetHolidayChan(g, 'defeat' + rename);
    ...
    //   parsed time value (0-2). Bounds the value and checks DB access.
      ...
      msgContent.includes('rbcontdow') ||
      msgContent.includes('rbcontfrog')
    ) //probably would break adams brain
    {
      var u_id = message.content.split(' ').slice(1, 2).join(' ').replace(' ', ''); //get the name for the role
    ...
    //   timestamp formatting. Assumes CSV format of specific columns.
      ...
      for (var i = 0; i < lines.length; i++) {
        ...
        else endstriin += 'left <#' + oldChannelID + '>';
        var timeint = parseInt(time / 1000);
        var timestriin = 'at <t:' + timeint + ':D> <t:' + timeint + ':T>';

        var resStrung =
          (newChannelID != null ? startstriiin + ' ' : '') +
          (oldChannelID != null ? endstriin + ' ' : '') +
          timestriin;

        // if starts with and remove it
        if (resStrung.startsWith('and ')) resStrung = resStrung.substring(4);
    ...
    //   mapping. Non-digit characters are removed before translation.
      ...
      for (var i = 0; i < message_id.length; i++) {
        ...
        if (num < validLetters.length) strg += validLetters[num];
        else strg += message_id[i];
      }

      // send the transposed message
      if (strg != '') message.author.send(strg);
    }
    ...
    // - Sends the raw `loggedUsersVCC.csv` file back to the author as an attachment.
      ...
      // send attachment
      message.author.send({
        files: [{ attachment: Buffer.from(csv), name: 'loggedUsersVCC.csv' }],
      });
    }
    // Handler: "trees"
    ...
    // - Sends both `fridayCounter.json` and `fridaymessages.json` as attachments.
      ...
      // send attachment
      message.author.send({
        files: [{ attachment: Buffer.from(fridayFile), name: 'fridayCounter.json' }],
      });
      var fridayMessagesFile = fs.readFileSync(babadata.datalocation + 'fridaymessages.json');
      // send attachment
      message.author.send({
        files: [{ attachment: Buffer.from(fridayMessagesFile), name: 'fridaymessages.json' }],
      });
    }
    ...
    //   author. Optionally accepts a numeric count (bounded to 50).
      ...
      // if blank message
      if (mesg === undefined) message.author.send('No DB Error Message');
      else {
        // loop through the first COUNT messages in lastDBError
        for (var i = 0; i < count; i++) {
          if (mesg[i] === undefined) break;
          ...
          for (var i = 0; i < entries.length; i++) {
            ...
            switch (k.targetType.toUpperCase()) {
              ...
              case 'GUILD':
                ...
                break;
              case 'UNKNOWN':
                if (actTxt == 'GuildVoiceStatusUpdate') outpiut += ' <#' + target.id + '>';
                break;
            }
