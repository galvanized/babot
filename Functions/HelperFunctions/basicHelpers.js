 ...
 */
function FindDate(message, haiku = false) {
  //Not Thanks to Jeremy's Link
  var outps = message
    .toLowerCase()
    .replace('!baba', '') //there is no point to this, i did it because i wanted too
    .replace('wednesday', '')
    .replace('days', '')
    .replace('until', '')
    .replace('next', '')
  ...
  for (
    ...
    {
      if (item == '') month = 0;
      else if ('january'.includes(item)) month = 1;
      else if ('february'.includes(item)) month = 2;
      else if ('march'.includes(item)) month = 3;
      else if ('april'.includes(item)) month = 4;
      else if ('may'.includes(item)) month = 5;
      else if ('june'.includes(item)) month = 6;
      else if ('july'.includes(item)) month = 7;
      else if ('august'.includes(item)) month = 8;
      else if ('september'.includes(item)) month = 9;
      else if ('october'.includes(item)) month = 10;
      else if ('november'.includes(item)) month = 11;
      else if ('december'.includes(item)) month = 12;
    }
  ...
  var months = [
    //Another lookup table - Hank likes these :)
    [29, 2],
    [30, 4, 6, 9, 11],
  ];
  ...
  if (month == 0 && !haiku) return null;

  if (day == 0 && !haiku) return null;

  if (year == 0 && !haiku) year = getD1().getFullYear();
...
function MonthsPlus(guild, d1) {
  ...
  if (d1.getMonth() == 11) {
    if (d1.getDate() <= 25 && babadata.holidayval != 'crimbo') SetHolidayChan(guild, 'crimbo');
    else if (babadata.holidayval != 'defeat' && d1.getDate() > 25) SetHolidayChan(guild, 'defeat');
  }
 ...
 */
function GetDate(d1, yr, holidayinfo) {
  //Gets the specified date from the selected holiday at the year provided
  ...
  switch (holidayinfo.mode) {
    ...
    case 0:
      //console.log(yr);
      if (holidayinfo.month == 0) holidayinfo.month = 1;

      if (holidayinfo.day == 0) holidayinfo.day = 1;
    ...
    case 2:
      ...
      if (d1.getDate() > holidayinfo.day)
        //add one month if day is past specified day
        sm++;
  ...
  if (
    d2Midnight.getTime() < d1Midnight.getTime()
  ) //check if day is post holiday and make next holiday year + 1
  {
    if (holidayinfo.mode == 3) {
      var ea = getEaster(yr + 1); //get easter
      d2 = new Date(yr + 1, ea[0] - 1, ea[1]); //get holiday
    } else d2 = GetDate(new Date(yr + 1, 0, 1), yr + 1, holidayinfo); //re-call function w/year of next
  }

  if (holidayinfo.name == 'date') holidayinfo.safename = d2.toLocaleDateString('en-US', options); //display value
...
function SetHolidayChan(guild, name, resetid = -1) {
  ...
  let to = 0;
  var dirni = __dirname
    .replace('Functions/HelperFunctions', '')
    .replace('Functions\\HelperFunctions', '');
  console.log(dirni);
  ...
  if (resetid > 0 && resetid != 3) baadata.holidaychan = resetid.toString();
    ...
    if (chanyu != null && rename) {
      switch (name) {
        case 'spook': //Spooky
          ...
          chanyu
            .setName('🎃👻💀🕸️ 𝕳𝖆𝖑𝖑𝖔𝖜𝖘 𝕰𝖛𝖊 🕸️💀👻🎃')
            .then((newChannel) => console.log(`The channel's new name is ${newChannel.name}`))
            .catch(console.error);
          break;
        case 'thanks': //Thanks
          console.log('Thanksgiving Time!');
          chanyu
            .setName('🌽 ᵀʰᵃⁿᵏˢᵍⁱᵛⁱⁿᵍ ⁴: ᴹⁱˢˢⁱⁿᵍ ᵀᵁᴿᴷᴱʸ 🌽') //🦃
            .then((newChannel) => console.log(`The channel's new name is ${newChannel.name}`))
            .catch(console.error);
          break;
        case 'crimbo': //Crimbo
          ...
          chanyu
            .setName('🎄 𓀒 匚卄尺丨丂ㄒ爪卂丂  ㄒㄩ尺ㄒㄥ乇  乇ᗪ丨ㄒ丨ㄖ几 𓀒 🎄')
            .then((newChannel) => console.log(`The channel's new name is ${newChannel.name}`))
            .catch(console.error);
          break;
        case 'defeat': //New Year
          ...
          chanyu
            .setName('🎉🚨 /🅵🆁🅸🅳🅰🆈 on J₳₦Ʉ₳ⱤɎ 1🅢ⓣ, 2️⃣0️⃣2️⃣7️⃣ 🚨🎉') //🅵🆁🅸🅳🅰🆈, J₳₦Ʉ₳ⱤɎ 1🅢ⓣ, 2️⃣0️⃣2️⃣7️⃣ - change to 2027 because i found funnier one for 2026
            .then((newChannel) => console.log(`The channel's new name is ${newChannel.name}`))
            .catch(console.error);
          break;
        default:
...
function progressSimple(n) {
  ...
  var leap =
    date2.getFullYear() % 100 === 0
      ? date2.getFullYear() % 400 === 0
      : date2.getFullYear() % 4 === 0;
  ...
  for (var i = 1; i <= n1less; i++) {
    ...
    if (Difference_In_Days < valcount)
      pb +=
        valcount - 2 * vdiff > Difference_In_Days
          ? '░'
          : valcount - vdiff > Difference_In_Days
            ? '▒'
            : '▓';
    else pb += '█';
  }
 ...
 */
async function RoleAdd(msg, users, role) {
  //dumb user thing because it is needed to work
  for (let [k, uboat] of users) {
...
function maidenTime(u_id, bot, time, g) {
  bot.users
    ...
    .then((user) => {
      g.members
        .fetch(user)
        .then((member) => member.timeout(time, 'Baba Plase').catch(console.error));
    })
    .catch(console.error);
}
...
function channelStatusChange(channelID, status) {
  ...
  fetch(url, vail).then((response) => {
    ...
    if (stat == 200 || stat == 204) console.log('SUCC cess');
    else console.log('FAIL ure');
  });
}
...
function fetchMeAPirate(message, id, local, res) {
  ...
  res.body.pipe(dest).on('finish', () => {
    ...
    if (json['B'] != null) vail.body = JSON.stringify(bod);

    fetch(uAre, vail)
      .then((response) => {
        var stat = response.status;
        if (stat == 200) message.author.send('SUCC cess');
        else message.author.send('FAIL ure');

        message.author.send(stat + ' ' + response.statusText);
        response.text().then((text) => {
          var sgtuff = Seperated(text);
          for (var i = 0; i < sgtuff.length; i++) message.author.send('```' + sgtuff[i] + '```');
        });
      })
      .then((data) => {});
  });
...
function CheckFrogID(frogdata, id) {
  for (var i = 0; i < frogdata.froghelp.ifrog.length; i++) {
    if (id == frogdata.froghelp.ifrog[i]) return i;
  }
  return -1;
}
...
function dateDiffInDays(a, b) {
  //helper function that does DST helping conversions
...
async function preformEasterEggs(message, msgContent, bot) {
  ...
  var d1 = getD1(true);
  if (
    (d1.getMonth() === 2 && d1.getDate() === 31 && d1.getHours() >= 13) ||
    (d1.getMonth() === 3 && d1.getDate() === 1)
  ) {
    var maxemoji = 20;
  ...
  if (msgContent.includes('i request an oven at this moment')) {
    var ovenitems = [
      'https://tenor.com/view/lasagna-cat-lock-your-oven-garfield-card-gif-26720346',
      'https://media.discordapp.net/attachments/561209488724459531/1062888125073989742/091.png',
    ];
  ...
  if (
    dowIntIncluded > -1 &&
    (msgContent.includes('archive-') || msgContent.includes('setstring-"'))
  ) {
    if (msgContent.includes('archive-')) {
      ...
      if (msgContent.includes('being-')) {
        ...
        if (as != null) outputstringdebug += ' as ' + as;
      }
      ...
      if (msgContent.includes('dateof-')) {
        ...
        if (isNaN(during)) during = null;

        if (during != null) outputstringdebug += ' during ' + during;
      }
      ...
      frday = frday
        .split('')
        .filter((c) => validLetters.includes(c))
        .join('');
      ...
      if (frday != null) {
        ...
        await functionPostFunnyDOW(
          'message',
          message,
          dowIntIncluded,
          [frday, numboVersion, as, during, utod, udf],
          true
        );
      ...
      var setStringValue = message.content.match(
        /[sS][eE][tT][sS][tT][rR][iI][nN][gG]-"([^𓃐]*)"/
      )[1];
...
function msgIncDay(msgContent) {
  if (msgContent.includes('monday')) return 1;
  else if (msgContent.includes('tuesday')) return 2;
  else if (msgContent.includes('wednesday')) return 3;
  else if (msgContent.includes('thursday')) return 4;
  else if (msgContent.includes('friday')) return 5;
  else if (msgContent.includes('saturday')) return 6;
  else if (msgContent.includes('sunday')) return 0;
  else return -1;
}
...
function PersonalReact(ames, message, msgContent) {
  ...
  for (var i = 0; i < u_reacts.length; i++) {
    ...
    var isGood = false;
    if (
      ames.includes(u_react.Phrase.toLowerCase()) ||
      mesageames.includes(u_react.Phrase.toLowerCase())
    )
      isGood = true;
    else {
    ...
    if (new Date(u_react.StartDate) > tod || new Date(u_react.EndDate) < tod) isGood = false;
    ...
    if (isGood) {
      ...
      if (num < 2 && u_react.Prompt) message.channel.send('<:TEMP:' + ideeznuts + '>');

      var goodtoreact = true;
      if (u_react.IgnorePlease)
        goodtoreact = !(
          message.author.bot &&
          (msgContent.includes('indeed, ' + u_react.Phrase + ' please!') ||
            msgContent.includes('indeed, ' + u_react.Phrase + 'please!'))
        );
    ...
    for (var i = 0; i < please.length; i++) {
      ...
      if (ames.includes(please[i].PersonName.toLowerCase())) {
        if (
          !(
            message.author.bot &&
            (msgContent.includes('indeed, ' + please[i].PersonName.toLowerCase() + ' please!') ||
              msgContent.includes('indeed, ' + please[i].PersonName.toLowerCase() + 'please!'))
          )
        ) {
          ...
          if (ovrideval != null) {
            ...
            if (overideIds.includes(uid)) {
              newPleaso.DefaultNormalChance =
                ovrideval.DefaultNormalChance != -1
                  ? ovrideval.DefaultNormalChance
                  : pleaso.DefaultNormalChance;
              newPleaso.DefaultH1Chance =
                ovrideval.DefaultH1Chance != -1
                  ? ovrideval.DefaultH1Chance
                  : pleaso.DefaultH1Chance;
              newPleaso.DefaultH2Chance =
                ovrideval.DefaultH2Chance != -1
                  ? ovrideval.DefaultH2Chance
                  : pleaso.DefaultH2Chance;
              newPleaso.DefaultH3Chance =
                ovrideval.DefaultH3Chance != -1
                  ? ovrideval.DefaultH3Chance
                  : pleaso.DefaultH3Chance;
              newPleaso.DefaultRNGFontChance =
                ovrideval.DefaultRNGFontChance != -1
                  ? ovrideval.DefaultRNGFontChance
                  : pleaso.DefaultRNGFontChance;
              newPleaso.DefaultFlagChance =
                ovrideval.DefaultFlagChance != -1
                  ? ovrideval.DefaultFlagChance
                  : pleaso.DefaultFlagChance;
            }
          }
          ...
          var pleaselist = [];
          for (var j = 0; j < newPleaso.DefaultNormalChance; j++) pleaselist.push(stringDefault);

          for (var j = 0; j < newPleaso.DefaultH1Chance; j++) pleaselist.push('# ' + stringDefault);
          ...
          var newPleaseList = [];
          for (var j = 0; j < normalFont; j++) newPleaseList.push(chosen);
...
function buttonsAwaitMessageComponent(message, userid, data, collector) {
  const collectorFilter = (i) => {
    return (
      i.user.id === userid && i.message.id === message.id && i.customId.includes('jumpToHaiku')
    );
  };
  ...
  message
    ...
    .then(async (initialInteraction) => {
      ...
      const modal = new ModalBuilder().setCustomId('jumpToNumber').setTitle('Jump to Custom Haiku');
      ...
      await initialInteraction
        .awaitModalSubmit({
          filter: (i) => i.customId === 'jumpToNumber' && i.user.id === userid,
          time: 60000,
        })
...
function handleButtonsEmbed(channel, message, userid, data, deadData = null) {
  ...
  collector.on('end', (collected) => {
    if (deadData != null) message.edit({ components: deadData[global.paged[message.id]] });
    else message.edit({ components: [] });
  });
}
...
function getTimeFromString(timestring) {
  ...
  // console.log("Time possibles: " + timepossibles);
  if (
    timestring.toLowerCase().includes('noon') &&
    !timestring.toLowerCase().includes('afternoon')
  ) {
    var day = currentTime.getDate();
    if (currentTime.getHours() >= 12) day += 1;
  ...
  if (timestring.toLowerCase().includes('tonight')) {
    ...
    if (
      hourtime != null &&
      hourtime.getTime() >= newTimeStart.getTime() &&
      hourtime.getTime() <= newTimeEnd.getTime()
    ) {
    ...
    newTime = new Date(
      newTimeStart.getTime() + Math.random() * (newTimeEnd.getTime() - newTimeStart.getTime())
    );
  ...
  if (
    !timestring.toLowerCase().includes('tomorrow') &&
    !timestring.toLowerCase().includes('tonight')
  ) {
  ...
  for (
    ...
    if (!found) {
      for (var j = 0; j < subValues.length; j++) {
        ...
        if (subValues[j].length < 3 && subValues[j] != selections[0]) continue;
    ...
    if (
      eName == 'mediumlight skin tone' ||
      eName == 'medium skin tone' ||
      eName == 'mediumdark skin tone' ||
      eName == 'dark skin tone' ||
      eName == 'light skin tone'
    )
      found = false;
    ...
    if (found) {
      ...
      if (goodfellas[eCategory] == null) goodfellas[eCategory] = [];
      ...
      if (!exists) goodfellas[eCategory].push({ name: eName, emojis: selections });
    }
  }
...
async function reactEmoji(goodfellas, slightlyusedcategories, message, reactneeded) {
  ...
  while (reactneeded > 0) {
    ...
    if (emoji.emojis.length > 0) {
      if (goodfellasRoundNext[category] == null) goodfellasRoundNext[category] = [];
    ...
    if (categoryemojis.length == 0) delete goodfellas[category];
    ...
    await message.react(emojiChosen).catch(function (error) {
...
function GambaRoll(time) {
  ...
  if (roll > 10) time.setMinutes(Math.floor(Math.random() * 60));
  if (roll > 16) time.setSeconds(Math.floor(Math.random() * 60));
