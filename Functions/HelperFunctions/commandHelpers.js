...
async function MakeImage(
  ...
  //Image Creation is now function
  var plu = false;
  if (weeks > 999999) {
    plu = true;
    weeks = 999999;
  ...
  Jimp.read(templocal + base).catch((err) => {
    base = 'date_base.png';
  });

  var baseImg = await Jimp.read(templocal + base).catch((err) => {
    base = 'date_base.png';
    textoverlay = true;
  });
  ...
  if (wednesdayoverlay == 'since' || wednesdayoverlay == 'sinces') {
    ...
    baseImg.print(font, 80, textlocal - 45, 'Wednesday' + s + ' Since', textoverlay ? 367 : 467);
  }
 ...
 */
function GetWhite(weekct) {
  //For frogs more than 100 weeks; "Retarded Lookup Table" - Hank 2021
  var wites = [
    ['1', 0, 1, 2, 4, 5, 6, 9, 10],
    ['2', 3, 8],
 ...
 */
async function BonusGenerator(bonus, im, templocal, weeks, ct, ln, moere) {
  ...
  if (bonus > 0) //creates images for more than 100 weeks
  {
    ...
    if (!kip) //not skipped
    {
      ...
      if (ct != 3 && weeks != 0)
        //only block white on values where last line wasnt 1000
      ...
      if (ln != 0) ni.composite(Twight, 0, 0); //get rid of black spots

      im = ni;
    }

    if (invisbonus)
      //reset bonus so no ecxtra 1 is printed
      bonus = 0;
...
function SingleHaiku(haiku, simnames, page, pagetotal) {
    ...
    outname =
      showname < 0.025
        ...
        : showname < 0.325
          ...
          : showname < 0.5
            ? haiku.DiscordName
            : GetSimilarName(simnames); // .85 > random discord name
    channame = showchan < 0.35 ? haiku.ChannelName : '';
    datetime = showdate < 0.5 ? new Date(haiku.Date) : '';
  }

  var signature = '';

  if (channame == '' && datetime == '')
    signature = outname; // randomness is great, dont judge
  else {
    signature = outname;
  ...
  var footobj = {
    text:
      '- ' +
      (!haiku.Accidental ? 'Purposful Haiku by ' : '') +
      signature +
      (page != null ? ' - Page ' + (1 + page) + ' of ' + pagetotal : ''),
    iconURL:
...
function EmbedHaikuGen(haiku, simnames) {
  ...
  for (var e = 0; e < haiku.length; e++) {
    ...
    var URLButton = new Discord.ButtonBuilder()
      .setURL(
        haiku[e].URL == null
          ? 'https://discord.com/channels/454457880825823252/979881683790733333/1183900512828006492'
          : haiku[e].URL
      )
 ...
 *          optionally `year`.
 */
function CheckHoliday(msg, holdaylist) {
  //checks if any of the holiday list is said in the message
  var retme = [];
  var ct = 0;
  for (var x in holdaylist) {
    ...
    for (var i = 0; i < hol.name.length; i++) {
      if (
        (msg == 'BIRTHDAY' && hol.safename == 'Birthday') ||
        msg == 'ALL' ||
        msg.toLowerCase().includes(hol.name[i].replace('[NY]', getD1().getFullYear() + 1))
      ) //checks if the holiday name is in the message
      {
        var item = {};
...
async function loadHurricaneHelpers() {
  if (global.dbAccess[1] && global.dbAccess[0]) await getHurricaneInfo();
...
function checkHurricane(hurricaneName, hurricaneJsonI) {
  ...
  var match =
    hurricaneJsonI.Name.toLowerCase() == hurricaneName.toLowerCase() ||
    (hurricaneJsonI.Name.charAt(0).toLowerCase() == huricaneNameLetter.toLowerCase() &&
      hurricaneJsonI.systemType != 'POTENTIAL TROPICAL CYCLONE' &&
      hurricaneJsonI.systemType != 'TROPICAL DEPRESSION') ||
...
async function checkHurricaneStuff(hurricanename) {
  ...
  for (var i in hurricaneJson) {
    ...
    if (checkHurricane(hurricanename, hurricaneJson[i])) {
      ...
      if (newDay > dbDay) {
        ...
        var saffirsympson = xml
          .split('<systemSaffirSimpsonCategory>')[1]
          .split('</systemSaffirSimpsonCategory>')[0];
      ...
      hurricaneJson[i].OverideText =
        hurricaneJson[i].Name.charAt(0).toLowerCase() == huricaneNameLetter.toLowerCase()
          ? { AltName: hurricanename }
          : hurricaneJson[i].Number == hurricanename
            ? { NumberSearch: hurricanename }
            : null;
  ...
  if (iNum == hurricaneJson.length) {
    ...
    while (xmlFound) {
      ...
      var url =
        'https://www.nhc.noaa.gov/storm_graphics/AT' +
        hurricanenameNum +
        '/atcf-al' +
        hurricanenameNum +
        thisYear +
      ...
      var imgURL =
        ...
        hurricanenameNum +
        thisYear +
        '_5day_cone_no_line_and_wind+png/';
      var xmlURL = url;
      var year = thisYear;
      ...
      var item = {
        ...
        Updated: true,
        OverideText:
          stormName.charAt(0).toLowerCase() == huricaneNameLetter.toLowerCase()
            ? { AltName: hurricanename }
            : number == hurricanename
              ? { NumberSearch: hurricanename }
              : null,
      };
