 ...
 */
async function setGrole(msg, rname) {
  //creates role and sets users
  /**
   * Create or fetch a role named `rname` and add it to users who reacted to `msg`.
   *
   * Behavior notes:
  ...
  try {
    ...
    setTimeout(function () {
      ...
      for (let [k, reee] of reactMap) {
        //iterate through all the reactions
        reee.users
          .fetch()
          .then((users) => {
...
async function setVote(msg) {
  //reacts to message with 👍 and 👎 for votes
  /**
   * Add thumbs-up and thumbs-down reactions to a message to enable voting.
   *
   * This is intentionally lightweight: callers simply invoke `setVote(msg)` and
...
async function setVBH(msg) {
  //reacts to message with emoji defined by babadata.emoji (in json file) for our implimentation that is the ban hammer emoji
  /**
   * React to a message with the configured moderation emoji (ban hammer).
   *
   * The emoji identifier is read from `babadata.emoji`. This helper performs
 ...
 * @returns {Promise<void>}
 */
async function movetoChannel(msg, channel, logchan, silent) {
  ...
  setTimeout(function () {
    msg.delete();
  }, waittime); //deletes the og message (delayed for the file transfer)
}
 ...
 * @returns {Promise<void>}
 */
async function DelayedDeletion(hiddenChan, img) {
  //download function used when the delay call is ran
  /**
   * Download an attachment to a temporary path and re-upload it into the
  ...
  download(url, tempFilePath, () => {
    //downloads the file to the system at tempfile location
    console.log('Done!');
  });
  ...
  var newAttch = new Discord.AttachmentBuilder(tempFilePath, {
    name: 'file' + suffix,
    description: 'Twas deleted from a place in time, ADAM PLEASE!',
  }); //makes a new discord attachment

  setTimeout(function () {
    hiddenChan.send({ files: [newAttch] });
  }, 2000); //sends the attachment (delayed by 1 sec to allow for download)

  setTimeout(function () {
    fs.unlinkSync(tempFilePath);
  }, 3000); //deletes file from local system (delayed by 3 sec to allow for download and upload)
}
