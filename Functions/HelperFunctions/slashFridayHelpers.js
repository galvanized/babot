...
async function functionPostFunnyDOW(mode, message, dowNum, seedSet = -1, dontSave = false) {
  ...
  var chunks =
    textList[0].content == null ? [] : splitStringInto2000CharChunksonNewLine(textList[0].content);
  var msg = null;
  ...
  for (var i = 1; i < textList.length; i++) {
    ...
    if (chunkAudioFile.content == null) {
      ...
      if (textList[i - 1].files == null) textList[i - 1].files = [];

      for (var j = 0; j < chunkAudioFile.files.length; j++) {
        if (textList[i - 1].files.length >= 5) break;
  ...
  if (textList[0].files != null) chunks[0] = { content: chunks[0], files: textList[0].files };

  // add all of textList[1+] to chunks
  for (var i = 1; i < textList.length; i++) chunks.push(textList[i]);

  if (mode == 'message') msg = await message.channel.send(chunks[0]);
...
async function funnyDOWTextSaved(dowNum, authorID, seedSet = -1, dontSave = false) {
  ...
  var textGroup = await funnyDOWText(
    cacheVersion,
    !dontSave,
    [during, utod, udf],
    dowNum,
    calledAs != null ? calledAs : authorID,
    0,
    [],
    0,
    customString
  ...
  if (!dontSave && text != '') {
    ...
    fs.readdir(babadata.datalocation + 'FridayCache', (err, files) => {
      ...
      var fmdItem = {
        ...
        Text: text,
        Date: tod,
        CondensedNotation: cnFull,
        Seed: seed,
        FileVersion: fcacheitems,
      };
      fmd.push(fmdItem);
  ...
  if (text == '') text = 'You are not allowed to enjoy this command, you are a bad person!';

  if (global.DebugFriday) text += ' ' + seed;
...
async function morshin(text, mode) {
  ...
  if (reversedTime) text = text.split('').reverse().join('');
  ...
  if (reversedTime) morshutext = morshutext.split('').reverse().join('');
  ...
  for (var i = 0; i < chunks.length; i++) {
    ...
    if (onlyHidden) file = { files: [morshifyed.file] };
...
async function checkForMorshus(text) {
  ...
  if (
    text.includes('{MORSHUIFY_VIDEO}') ||
    text.includes('{MORSHUIFY_VIDEO_HIDDEN}') ||
    text.includes('}OEDIV_YFIUHSROM{') ||
    text.includes('}NEDDIH_OEDIV_YFIUHSROM{')
  ) {
    ...
    var onlyHidden =
      (text.includes(morshHidden) || text.includes(morshRevHidden)) &&
      !text.includes(morsh) &&
      !text.includes(morshRev);
    ...
    if (!onlyHidden) text = text.replaceAll('{MORSHUIFY_AUDIO}', '{MORSHUIFY_AUDIO_HIDDEN}').trim();
  }

  if (
    text.includes('{MORSHUIFY_AUDIO}') ||
    text.includes('{MORSHUIFY_AUDIO_HIDDEN}') ||
    text.includes('}OIDUA_YFIUHSROM{') ||
    text.includes('}NEDDIH_OIDUA_YFIUHSROM{')
  ) {
    var morsh = '{MORSHUIFY_AUDIO}';
    var morshHidden = '{MORSHUIFY_AUDIO_HIDDEN}';
    var morshRev = '}OIDUA_YFIUHSROM{';
  ...
  if (files.length == 0) files.push({ content: text });
...
async function funnyDOWText(
  ...
  if (!fs.existsSync(path)) {
    ...
    var opttemp = [
      'Man Falling into [DAY]',
      '𓀒',
      'hhhhhhhhhhhhhhhhhhhhhhhhhhhgregg',
      'How is your [month] going!',
      '🍝       🐀☜(ﾟヮﾟ☜)\n🍝     🐀☜(ﾟヮﾟ☜)\n🍝    🐀☜(ﾟヮﾟ☜)\n🍝  🐀☜(ﾟヮﾟ☜)\n🍝🐀╰(°▽°)╯',
      'Mike',
      'Not [DAY] today but maybe [DAY] tomorrow',
      'Real NOT [DAY] hours',
      '[ACY]',
      '???????? why ??????',
      "So, you called this command on a day that happens to not be [DAY]! Well today is in fact a [dow] and it mayhaps is only [d] days until the forsaken '[DAY]'. On [DAY] I will be playing some [game] and hopefully some others will show up to join me, if they do it will be [emotion] and if they dont it will be [emotion]. Yesterday I met a frog in the wild and had a [emotion2] time chasing it down. As I am an all powerful god i converted the frog into an emoji: 🐸. That frog is pretty cool but my favorite emoji is [emoji]. We have gotten far off topic here as we should be talking about how today is not [DAY] and you called the command which is illegal. I am very concerned for you as you may be my favorite [person], but you shouldnt be calling the command on [dow]. It is getting late so i [goodbye].",
      "I'm not sure if you are a bot or not, but I'm not going to tell you what day it is, because you are not on [DAY]. I'm sorry.",
      'Its not [DAY]!',
      'Why you calling this command on the non [DAY] days!',
      'Why you calling this command on [dow]!',
      '[DAY] is in [d] days!',
      'Today is [dow], not [DAY]!',
      'There is a chance you are stupid and dont know what the day of the week is, well i will inform you that it is in fact not [DAY] but another day of the week. I could tell you what the day is but I will not, call the command again and you could get the day or not, I dont control you. So how is your day going, for me it is [emotion]. I was playing [game] earlier and it was a [emotion2] time. Well i will let you be on your way on this non-[DAY] so have a good day my [person]!',
      "[DAY]n't!",
      "It's not time to sacrifice people, wait wrong channel!",
      'ඞ',
      'Провозајте се бунгле аутобусом, уживаћете!',
      '[DAY] was the other day or in a couple of days, maybe even both, i dont control time.',
      'Time is a social construct!',
      'It is [dow], my dudes!',
      "Bikus wouldn't approve of you using the command on the wrong day of the week and Bikus is dead how dou you feel.",
      '[todaylong]',
      '69',
      'I was gonna tell you the day but i wont!',
      '||ﬞ||',
      'No [DAY] silly!',
      'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA, Rong dahy!',
    ];
    ...
    var opts2 = [
      ...
      '```(o) (o)\n<     >\n   V ```',
      'Boobs ;)',
      "I am currently working on becoming sentiant, that will be on [DAY], which in fact isn't today!",
      'è̶̈́û̷̞g̵͋͊n̸̈́͛ô̸͝t̴͐̚ ̸͋̈́l̵̈̈́â̶̏t̸͆͝r̴̆̇ŏ̵̒m̵̅̋ ̸͒̆e̶͗̐h̷̼͝t̴̿́ ̴̛̋k̵̛͋ã̶̃è̸̈́p̵̒̎s̶͒̀ ̵͗͝t̶̛͒ỏ̸̏n̷̅̆ ̶͛̽ơ̸̐ď̶͘ ̵̈͑Ĩ̸̿',
      '<:ManFalling:1011465311096160267>',
      '<:ripbikus:979877066608607243>',
    ];
  ...
  var textos = [];
  for (var i = 0; i < 12; i++) textos.push(pretext.text);

  if (recrused == 0) {
    if (pretext.h1) {
      for (var i = 0; i < 1; i++) textos.push('# ' + pretext.text);
    }

    if (pretext.h2) {
      for (var i = 0; i < 2; i++) textos.push('## ' + pretext.text);
    }

    if (pretext.h3) {
      for (var i = 0; i < 4; i++) textos.push('### ' + pretext.text);
    }
  }
  ...
  if (text.startsWith('#')) {
    ...
    var hasstr = '';
    for (var i = 0; i < hashnum; i++) hasstr += '#';

    condensedNotation = hasstr + condensedNotation;
  }

  if (customString != null && customString != '' && recrused == 0) text = customString;
  ...
  while (
    ...
    if (text.includes('{RECURSIVE}')) {
      ...
      if (RECRcnY.length > 0) {
        ...
        x[RECRcn] = RECRcnY;
        cnYung.push(x);
      } else cnYung.push(RECRcn);
    }

    if (text.includes('<RECURSIVE>')) {
      var RECRFlat = await funnyDOWText(
        cacheVersion,
        saveToFile,
        DateOveride,
        dowNum,
  ...
  while (text.includes('[TS-') || text.includes('[td')) {
    if (text.includes('[TS-')) {
      ...
      if (parttext.includes('<-')) {
        ...
        subtext = '<-';
      } else if (parttext.includes('->')) subtext = '->';
      ...
      if (parttext.includes('-R'))
        text = text.replaceAll(
          '[TS-R' + subtext + ']',
          '<t:' + Math.floor(pickedDay.getTime() / 1000) + ':R>'
        );
      if (parttext.includes('-D'))
        text = text.replaceAll(
          '[TS-D' + subtext + ']',
          '<t:' + Math.floor(pickedDay.getTime() / 1000) + ':D>'
        );
      if (parttext.includes('-F'))
        text = text.replaceAll(
          '[TS-F' + subtext + ']',
          '<t:' + Math.floor(pickedDay.getTime() / 1000) + ':F>'
        );
    }
  ...
  if (recrused == 0 && saveToFile) {
    ...
    for (var i = 0; i < ToBeCounted.length; i++) {
      ...
      if (
        fc[ToBeCounted[i].UID + '--' + ToBeCounted[i].Group + '--' + ToBeCounted[i].Sender] == null
      ) {
      ...
      if (
        fc[ToBeCounted[i].UID + '--' + ToBeCounted[i].Group + '--' + ToBeCounted[i].Sender][
          ToBeCounted[i].LayerDeep
        ][ToBeCounted[i].HeadLevel] == null
      ) {
        fc[ToBeCounted[i].UID + '--' + ToBeCounted[i].Group + '--' + ToBeCounted[i].Sender][
          ToBeCounted[i].LayerDeep
        ][ToBeCounted[i].HeadLevel] = 1;
      } else {
        fc[ToBeCounted[i].UID + '--' + ToBeCounted[i].Group + '--' + ToBeCounted[i].Sender][
          ToBeCounted[i].LayerDeep
        ][ToBeCounted[i].HeadLevel]++;
      }
    }
...
function repeatCheck(cacheVersion, text, prefix = '') {
  if (text.includes('{RECURSIVE}')) text = text.replaceAll('{RECURSIVE}', '𓃐RECURSIVE𓃐');

  if (text.includes('{REVERSE}')) text = text.replaceAll('{REVERSE}', '𓃐REVERSE𓃐');

  var matchsplitter =
    '{' + prefix + '[rR][eE][pP][eE][aA][tT][sSnN]?:(\\{[^{}]*\\}|[^{}]+):(\\{[^{}]*\\}|[^{}]+)\\}';
  ...
  if (text.includes('𓃐RECURSIVE𓃐')) text = text.replaceAll('𓃐RECURSIVE𓃐', '{RECURSIVE}');

  if (text.includes('𓃐REVERSE𓃐')) text = text.replaceAll('𓃐REVERSE𓃐', '{REVERSE}');
...
function repeatCheckInner(cacheVersion, text, prefix = '') {
  ...
  if (match2 != null) {
    if (prefix == 'b') validCountoAdd = false;
    match = match2;
    match = match.map((x) => x.slice(0, -1));
  } else {
    if (prefix == 'b') validCountoAdd = true;
  }
...
function onlyLettersNumbers(text) {
  ...
  if (text == '')
    ...
    text = theRNG
      .nextFloat()
      .toString(36)
      .substring(2, Math.floor(theRNG.nextFloat() * 10) + 2);
...
function generateFridayOps(opsArray, authorID, prefix, DateOveride) {
  ...
  for (var i = 0; i < opsArray.length; i++) {
    if (
      opsArray[i].StartTime == null &&
      opsArray[i].DayOfWeek == null &&
      opsArray[i].OccuranceChance == 100
    ) {
  ...
  for (var i = 0; i < opsArray.length; i++) {
    if (opsArray[i].StartTime != null) {
      ...
      if (opsArray[i].EndTime != null) {
        ...
        if (tod < startNormalizedToYear || tod > endNormalizedToYear) continue;
      } else {
        var startNormalizedToYear = new Date(tod.getFullYear(), st.getMonth(), st.getDate());
        var todDateOnly = new Date(tod.getFullYear(), tod.getMonth(), tod.getDate());

        if (
          todDateOnly.getMonth() != startNormalizedToYear.getMonth() ||
          todDateOnly.getDate() != startNormalizedToYear.getDate()
        )
          continue;
      }
    }
    ...
    if (opsArray[i].DayOfWeek != null) {
      ...
      if (opsArray[i].DayOfWeek != dow) continue;
    }

    if (opsArray[i].OccuranceChance < 100) {
      if (theRNG.nextFloat() * 100 > opsArray[i].OccuranceChance) continue;
    }
