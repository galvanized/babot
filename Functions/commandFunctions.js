...
const {
  ...
  ObtainDBHolidays,
  NameFromUser,
} = require('./Database/databaseandvoice.js');
const {
  FindDate,
  GetDate,
  dateDiffInDays,
  getTimeFromString,
  progressSimple,
} = require('./HelperFunctions/basicHelpers.js');
const {
  CheckHoliday,
  FindNextHoliday,
  MakeImage,
  ...
  checkHurricaneStuff,
  monthFromInt,
} = require('./HelperFunctions/commandHelpers.js');
...
async function babaFriday(isFake = false) {
  ...
  if (!isFake && global.BirthdayToday != null) {
    ...
    alttext =
      'Baba Friday Image, As it is ALWAYS Friday!\nCelebrating: ' +
      names.join(' and ') +
      ' Edition!';

    newFile = new Discord.AttachmentBuilder(await image.getBufferAsync(Jimp.MIME_JPEG), {
      name: 'Friday.jpg',
      description: alttext,
    });
  } else {
    newFile = new Discord.AttachmentBuilder(templocal + 'Friday.jpg', {
      name: 'Friday.jpg',
      description: alttext,
    });
  }
...
function babaRNG(min, max, spoiler) {
  ...
  return {
    content: 'Your Random Number is: ' + (spoiler ? '||' : '') + num + (spoiler ? '||' : ''),
  };
}
...
function babaPlease() {
  ...
  if (num < 2) return { content: 'AAAAAAAAAAA' };
  else if (num < 15) return { content: 'BABA IS HAPPY!' };
  else if (num < 45) return { content: 'BABA IS THANKS!' };
  else if (num < 68) return { content: 'BABA IS PLEASED!' };
  else if (num == 69) return { content: 'Nice!' };
}
...
function babaVibeFlag() {
  ...
  var locals = [
    //another thing hank doesnt like, but it is needed
    [0, 1, 2, 3, 4, 5, 6],
    [6, 5, 4, 3, 2, 1, 0],
    [1, 3, 5, 0, 2, 4, 6],
    [0, 2, 4, 6, 5, 3, 1],
  ...
  var newFile = new Discord.AttachmentBuilder(
    babadata.datalocation + 'Flags/' + 'Night_Shift_' + sood + '.png',
...
function babaYugo() {
  ...
  function babaYugo() {
    ...
    var yugo = new Discord.AttachmentBuilder(
      babadata.datalocation + 'Yugo/' + num.toString() + '.jpg',
      {
        name: 'Yugo.jpg',
        description:
          'This is yugo number ' +
...
function babaRepost() {
  ...
  function babaRepost() {
    ...
    var reppy = new Discord.AttachmentBuilder(
      babadata.datalocation + 'Repost/' + num.toString() + '.png',
      {
        name: 'Repost.png',
        description:
          'This is repost number ' + num + '!\nWhen will jeremy finish his report detector?',
      }
    );
    return { files: [reppy] };
  }
...
function babaHaikuLinks(cont) {
  ...
  for (var i = 0; i < cont.length; i++) {
    ...
    if (cont[i].components[0].components[cpu.length - 1].data.style == 5) {
      ...
      var URLButton = new Discord.ButtonBuilder()
        .setURL(cont[i].components[0].components[cpu.length - 1].data.url)
...
function babaHaikuEmbed(purity, mode, msgContent, pagestuff) {
   ...
   * @returns {Array|Object} Embed objects or message payloads ready to send.
   */
  if (mode != 4) msgContent = normalizeMSG(msgContent);
  else {
    for (var i = 0; i < msgContent.length; i++) {
      if (typeof msgContent[i] == 'string') msgContent[i] = normalizeMSG(msgContent[i]);
    }
  }
  ...
  if (purity) {
    ...
    if (result == null) {
      haifou = true;
      return [
        { content: 'Result was null, something may have gone wrong, or no Haikus were found!' },
      ];
    }

    hpl = FormatPurityList(
      result,
      msgContent[6] == 'chans' ? true : msgContent[6] == 'dates' ? 2 : false,
      pagestuff
    );
    ...
    if (!haifou) return EmbedPurityGen(hpl, bonust, bonupr, pagestuff);
  } else {
...
function EmbedPurityGen(hpl, bonust, bonupr, pagestuff, msgContent) {
  ...
  for (var e = 0; e < pagetotal; e++) {
    ...
    if (msgContent != undefined) {
      ...
      if (kword != null) cont += 'Containing: ' + kword.split(' ').join(', ') + '\n';

      if (sd != null || ed != null) {
        if (sd != null) startDate = FindDate(sd);
        if (ed != null) endDate = FindDate(ed);
    ...
    if (pagetotal > 1) {
      ...
      var pButton = new Discord.ButtonBuilder()
        .setCustomId('page' + (e - 1))
        .setLabel('Previous')
        .setStyle(1);
      var nButton = new Discord.ButtonBuilder()
        .setCustomId('page' + (1 + e))
        .setLabel('Next')
        .setStyle(1);
      if (e == 0) {
...
function babaJeremy() {
   ...
   */
  var data = JSON.parse(
    fs.readFileSync(babadata.datalocation + 'data.json', { encoding: 'utf8', flag: 'r' })
  );
...
async function babaUntilHolidays(msgContent, author, DOWChosen) {
  ...
  if (IsDate != null) IsHoliday.push(IsDate);
  ...
  if (msgContent.includes('next event')) {
    ...
    for (
      var i = 0;
      i < hols.length;
      i++ // Add all the events to the list that are coming up
    )
  ...
  if (msgContent.includes('next birthday')) {
    ...
    for (
      var i = 0;
      i < hols.length;
      i++ // Add all the birthdays to the list that are coming up
    )
      IsHoliday.push(hols[i]);
  }
  ...
  if (IsHoliday.length > 0) {
    ...
    for (
      var i = 0;
      i < IsHoliday.length;
      i++ //loop through the holidays that are requested
    ) {
      ...
      if (isNaN(d2)) {
        var fronge = new Discord.AttachmentBuilder(
          babadata.datalocation + 'FrogHolidays/error.png',
          { name: 'ErrorFrog.png', description: 'Brug, run commands better bud hee!' }
        );

        outs.push({
          content: 'The date does not exist so BABA will give you ERROR frog!',
          files: [fronge],
        });
        continue;
      }
      ...
      if (DOWChosen == '00') DOWChosen = '0' + (d1.getDay() + 1);

      if (DOWChosen == '01') dowtext = 'sunday';
      else if (DOWChosen == '02') dowtext = 'monday';
      else if (DOWChosen == '03') dowtext = 'tuesday';
      else if (DOWChosen == '04') dowtext = 'wednesday';
      else if (DOWChosen == '05') dowtext = 'thursday';
      else if (DOWChosen == '06') dowtext = 'friday';
      else if (DOWChosen == '07') dowtext = 'saturday';

      if (msgContent.includes(dowtext)) showwed = true;
      ...
      if (msgContent.includes('when is')) //outputs the next occurance of the event
      {
        ...
        if (
          msgContent.includes('when isnt') ||
          msgContent.includes('when is not') ||
          msgContent.includes("when isn't")
        ) {
      ...
      {
        var bonustext =
          holidayinfo.year != undefined && holidayinfo.year != 0 ? ' ' + holidayinfo.year : '';
        var dowtext =
          holidayinfo.safename +
          bonustext +
          ' is on ' +
          d2.toLocaleDateString('en-US', { weekday: 'long' }); //future text
      ...
      if (msgContent.includes('days until')) //custom days until text output - for joseph
      {
        ...
        if (int != 0)
          dutext +=
            ...
            d2.getTime() / 1000 +
            ':R>' +
            (int > 31 ? ' which is in ' + int + ' day' + (int == 1 ? '' : 's') : '!');
        else {
          dutext += 'Today!';
          showwed = true;
        }
      ...
      if (msgContent.includes('days since')) //custom days until text output - for joseph
      {
        ...
        if (int != 0)
          dutext +=
            ...
            d2.getTime() / 1000 +
            ':R>' +
            (int > 31 ? ' which was ' + int + ' day' + (int == 1 ? '' : 's') : ' ago!');
        else {
          dutext += 'Today!';
          showwed = true;
        }
      ...
      if (msgContent.includes('eves')) {
        ...
        for (var j = 0; j < HundredGroups; j++) {
          var newInt = 450;
          if (j == HundredGroups - 1 && int % 450 != 0) newInt = int % 450;
        ...
        else additionaltext += holidayinfo.safename + bonustext + ' is Today ' + eves + '!\n';
      }
      ...
      if (weeks < 0.3)
        //for when it is the week before and set to .142
        weeks = 0;
      ...
      if (DOWChosen == '04') {
        ...
        if (weeks == 1) wednesdayoverlay = 'Wednesday_Single.png'; //one week means single info

        if (d2 < d1) {
          wednesdayoverlay = 'sinces';
          if (weeks == 1) wednesdayoverlay = 'since';
        }
        ...
        if (d1.getTime() - d2.getTime() == 0) {
          ...
          var custom = false;
          if (holidayinfo.name == 'date') custom = true;
          else {
            try {
          ...
          if (custom) {
            ...
            Jimp.read(templocal + outputname)
              ...
              .then(function (font) {
                loadedImage
                  .print(font, 190, 20, holidayinfo.safename)
    ...
    for (var j = 0; j < templocationslist.length; j++) {
      var newAttch = new Discord.AttachmentBuilder(templocationslist[j], {
        name: IsHoliday[j].name + '.png',
        description: 'It is ' + IsHoliday[j].name + ', my dudes',
      ...
      try {
        ...
        var newAttch = new Discord.AttachmentBuilder(templocal + 'error.png', {
          name: 'error.png',
          description: 'It is error time, my dudes! Brug why you erroring baba?',
        }); //makes a new discord attachment (default fail image)
      }
    ...
    if (d1.getDay() == 3 && (mode == '00' || mode == '04'))
      ...
      else outs.push({ content: 'FUNNYDOW' });
    }
  }

  if (outs.length == 0) outs.push({ content: 'Baba Broke Getting that Event!' });
...
async function babaWhomst(user) {
  ...
  if (result == null) return 'Baba could not find that user!';
...
async function babaHurricane(hurricanename, callback) {
  ...
  if (hurricanename != '' && hurricanename != null) {
    ...
    if (hurricaneInfo != null) {
      hfull =
        ' for ' +
        (hurricaneInfo.Category == 'N/A'
          ? hurricaneInfo.Type
          : hurricaneInfo.Category + ' ' + hurricaneInfo.Type) +
        ' ' +
        hurricaneInfo.Name;
...
function babaWeather(mode, city, callback) {
  ...
  if (mode == 'four') url = 'https://wttr.in/' + cityUnderscore + '.png?u';
  else if (mode == 'deets') url = 'https://v2.wttr.in/' + cityUnderscore + '.png?u';
...
function babaAurora(time, callback) {
  var url =
    'https://services.swpc.noaa.gov/experimental/images/aurora_dashboard/' +
    time +
    '_static_viewline_forecast.png';
...
function babaGoodberrys(callback) {
  publicGoogleCalendar = new PublicGoogleCalendar({
    calendarId: '24gbb7942jsn557e7l93in7itjmo5lqj@import.calendar.google.com',
  });
