...
const {
  ...
  getUserIDFromID,
} = require('./HelperFunctions/remindersByBaba.js');
const {
  sqlEscapeStringThingforAdamBecauseHeWillDoanSQLInjectionOtherwise,
} = require('./HelperFunctions/dbHelpers.js');
...
async function contextInfo(interaction, bot) {
  ...
  if (commandName === 'Delete') {
    ...
    const modal = new ModalBuilder().setCustomId('movetoModal').setTitle('Move Message');
  ...
  if (cid === 'movetoModal') {
    ...
    if (reminder == null)
      await interaction.reply({
        content: 'Reminder not found, it may have been deleted/completed already',
        ephemeral: true,
      });
    else {
    ...
    if (massamage != null) {
      ...
      if (finalComps != null) delete reminderList.finalComponents;
...
async function buttonInfo(interaction, bot) {
  ...
  if (cid.startsWith('editrem-')) {
    ...
    if (reminder == null) {
      await interaction.reply({
        content: 'Reminder not found, may have been deleted/completed',
        ephemeral: true,
      });
      return;
    }
    ...
    var timeString = theDate.toLocaleTimeString('en-US', {
      hour: 'numeric',
      minute: 'numeric',
      second: 'numeric',
    });
    var dateString = theDate.toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
    });
    ...
    for (var i = 0; i < cont.length; i++) cont[i].components = null;
    ...
    var r2 = new Discord.ActionRowBuilder().addComponents(
    ...
    var r3 = new Discord.ActionRowBuilder().addComponents(
    ...
    var r4 = new Discord.ActionRowBuilder().addComponents(
      new Discord.ButtonBuilder()
        .setCustomId('generateHaikuList')
        .setLabel(
          cid === 'purity'
            ? 'Generate Purity Score List'
            : cid === 'haiku'
              ? 'Open Haiku Search Form'
              : 'Open Multi-Haiku Search Form'
        )
        .setStyle(1)
    );

    if (cid === 'purity') contenenent.components = [row, r2, r3, r4];
    else contenenent.components = [r2, r3, r4];

    interaction.editReply(contenenent);
  } else if (cid === 'generateHaikuList') {
    var msg = interaction.message;
    if (global.interactions[msg.id] === undefined)
      global.interactions[msg.id] = {
        puritymode: null,
        personList: null,
        channelList: null,
        purity: false,
    ...
    if (pMode === null && global.interactions[msg.id]['purity'] === true) {
      ...
      if (pMode === 'chans') pMode = 'Channels';
      else if (pMode === 'users') pMode = 'Users';
      else if (pMode === 'dates') pMode = 'Dates';

      const modal = new ModalBuilder()
        .setCustomId(
          'haiku-' +
            (global.interactions[msg.id]['purity']
              ? 'Purity'
              : global.interactions[msg.id]['all']
                ? 'All'
                : 'Haiku') +
            '-' +
            msg.id
      ...
      const keywordInput = new TextInputBuilder()
        ...
        .setValue(
          global.interactions[msg.id]['keyword'] != null
            ? global.interactions[msg.id]['keyword']
            : ''
        );
      ...
      const startDateInput = new TextInputBuilder()
        ...
        .setValue(
          global.interactions[msg.id]['sdate'] != null ? global.interactions[msg.id]['sdate'] : ''
        );
      ...
      const endDateInput = new TextInputBuilder()
        ...
        .setValue(
          global.interactions[msg.id]['edate'] != null ? global.interactions[msg.id]['edate'] : ''
        );
      ...
      const personInput = new TextInputBuilder()
        ...
        .setValue(
          global.interactions[msg.id]['person'] != null ? global.interactions[msg.id]['person'] : ''
        );
...
async function stringSelectInfo(interaction, bot) {
  ...
  if (cid === 'puritymode') {
    if (global.interactions[interaction.message.id] === undefined)
      global.interactions[interaction.message.id] = {
        puritymode: null,
        personList: null,
        channelList: null,
        purity: true,
