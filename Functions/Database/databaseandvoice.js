...
function FilterDate(haikuList, BFE, Year, Month, Day) {
  if (BFE == 'Exact') {
    var filteredList = haikuList.filter(function (haiku) {
      ...
      return (
        (Year == null ? true : haikuDate.getFullYear() == Year) &&
...
function HaikuSelection(messageTerm, mode) {
  ...
  if (mode == 1) {
    ...
    haikuList = FilterDate(
      haikuList,
      'Exact',
      Year == 0 ? null : Year,
      Month == 0 ? null : Month - 1,
      Day == 0 ? null : Day
    );
  } else if (mode == 5) {
    ...
    if (sd != null) startDate = FindDate(sd, true);
    if (ed != null) endDate = FindDate(ed, true);
    ...
    if (startDate != null) {
      if (endDate != null) {
        ...
        haikuList = FilterDate(
          haikuList,
          'After',
          startDate.year == 0 ? null : startDate.year,
          startDate.month == 0 ? null : startDate.month - 1,
          startDate.day == 0 ? null : startDate.day
        );
        haikuList = FilterDate(
          haikuList,
          'Before',
          endDate.year == 0 ? null : endDate.year,
          endDate.month == 0 ? null : endDate.month - 1,
          endDate.day == 0 ? null : endDate.day
        );
      } else {
        haikuList = FilterDate(
          haikuList,
