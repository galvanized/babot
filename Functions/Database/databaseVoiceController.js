  ...
  if (timeoutCT > 1) {
    ...
    timeoutFix = setTimeout(async function () {
      ...
      if (pingged == 'ERROR') {
        ...
        timeoutCT++;
        console.log(
          timestring + ': Database Connection Failed, Retrying in 60 seconds -> ' + timeoutCT,
          false,
          true
        );
        ...
        console.log(
          timestring +
            ': Database Connection was null, attempted reconnect, Retrying in 60 seconds -> ' +
            timeoutCT,
          false,
          true
        );
...
function DMMePlease(sourceMessage, consoledlog = true) {
  if (consoledlog) console.log('DMMePlease: ' + sourceMessage, false, true);

  var guildID = babadata.testing === undefined ? '454457880825823252' : '522136584649310208';
  var logThreadChanID =
    babadata.testing === undefined ? '1337944450084769876' : '1337943563996106915';
  var channelOfThread =
    babadata.testing === undefined ? '509401300874690590' : '757071872721682594';

  global.Bot.guilds
    .fetch(guildID)
    .then(async (guild) => {
      guild.channels
        .fetch(channelOfThread)
        .then((channel) => {
      ...
      guild.channels
        ...
        .then((channel) => {
          channel.threads.fetch(logThreadChanID).then((thread) => {
            thread.send({
              files: [
                { attachment: Buffer.from(JSON.stringify(filedata, null, 2)), name: filename },
              ],
              content: description,
            });
          });
  ...
  if (!change.includes('user')) {
    ...
    if (change == 'create') {
      ...
      callSQLQuery(qurey)
        .then((result) => {})
        .catch((err) => {
          DMMePlease('Error Creating Event: ' + err);
        });
      ...
      callSQLQuery(qurey)
        .then((result) => {})
        .catch((err) => {
          DMMePlease('Error Deleting Event: ' + err);
        });
      ...
      callSQLQuery(qurey)
        .then((result) => {})
        .catch((err) => {
          DMMePlease('Error Updating Event: ' + err);
        });
    }
  } else {
    var uid = user.id;
    ...
    if (change == 'useradd') {
        ...
        .then((result) => {
          if (result.affectedRows == 0) {
            ...
            callSQLQuery(innrquery)
              .then((result) => {})
              .catch((err) => {
                DMMePlease('Error Adding User to Event: ' + err);
              });
          }
        })
        .catch((err) => {
          DMMePlease('Error Updating User in Event: ' + err);
        });
      ...
      callSQLQuery(query)
        .then((result) => {})
        .catch((err) => {
          DMMePlease('Error Removing User from Event: ' + err);
        });
    }
  }
}
    ...
    callSQLQuery(query)
      ...
      .catch((err) => {
        if (
          err != null &&
          err.sqlMessage != null &&
          err.sqlMessage.includes('voiceactivity_ibfk_1')
        ) {
          console.log('Error: ' + err.sqlMessage, false, true);
          guild.channels
            .fetch(channelID)
            .then((channel) => {
              checkAndCreateChannel(channelID, channel.name)
                .then(() => {
          ...
          err != null &&
          err.sqlMessage != null &&
          err.sqlMessage.includes('voiceactivity_ibfk_2')
        ) {
          console.log('Error: ' + err.sqlMessage, false, true);
          guild.members
            .fetch(userID)
            .then((user) => {
              CheckAndCreateUser(userID, user.user.username)
                .then(() => {
                  userVoiceChange(queryz, userID, channelID, guild, subtext)
                    .then((result) => {
                      resolve(result);
...
function userJoinedVoice(userID, channelID, guild, overideTime = null) {
  var PromisedUserJoined = new Promise((resolve, reject) => {
    ...
    userVoiceChange(q, userID, channelID, guild, 'JoinVoice')
      .then((result) => {
        resolve(result);
      })
      .catch((err) => {
...
function logVCC(newMemberID, newChannelID, oldMemberID, oldChannelID, guildID, timeoveride = null) {
  ...
  if (timeoveride != null) time = timeoveride.getTime();
  ...
  fs.appendFileSync(
    babadata.datalocation + 'loggedUsersVCC.csv',
    newMemberID +
      ',' +
      newChannelID +
      ',' +
...
function clearVCCList() {
  ...
  async function processLinesSequentially(lines) {
    for (let i = 0; i < lines.length; i++) await saveStuff(lines[i], i);
  }
 ...
 * @param {string} guildID - Discord guild snowflake ID.
 */
function voiceChannelChangeLOGGED(
  newMemberID,
  oldMemberID,
  newChannelID,
  oldChannelID,
  ...
  global.Bot.guilds.fetch(guildID).then(async (guild) => {
    const VCCChangeAsync = async function () {
      if (
        ...
        userOptValue(guild, newMemberID, 'voice')
      ) {
...
async function PickThePerfectUsername(member, order = ['N', 'C', 'G', 'U'], regexTrim = true) {
  ...
  if (nName == null) nName = '';
  if (cahcedName == null) cahcedName = '';
  if (gName == null || gName == 'No One') gName = '';
  if (uName == null) uName = '';
...
function voiceChannelChange(newMember, oldMember) {
  const VCCChangeAsync = async function () {
    ...
    var shadowRealmChannel =
      babadata.testing === undefined ? '454464489681715200' : '1240062704966832209';
    ...
    if (newUserChannel == shadowRealmChannel || oldUserChannel == shadowRealmChannel) {
      ...
      var usersInShadowRealm = await guild.channels
        .fetch(shadowRealmChannel)
        .then((channel) => channel.members.map((member) => member.id));
      if (usersInShadowRealm.length == 0) channelStatusChange(shadowRealmChannel, '');
      else {
        var userNamedList = [];
        for (var i = 0; i < usersInShadowRealm.length; i++) {
          var userID = usersInShadowRealm[i];
          var userName = await PickThePerfectUsername(
        ...
        if (userNamedList.length > 1)
          userNamedListString =
            userNamedListString.substring(0, userNamedListString.lastIndexOf(',')) +
            (userNamedList.length > 2 ? ',' : '') +
            ' and' +
            userNamedListString.substring(userNamedListString.lastIndexOf(',') + 1);
        else if (userNamedList.length == 1) userNamedListString = userNamedList[0];

        userNamedListString +=
          (userNamedList.length > 1 ? ' are' : ' is') +
          ' Sleeping, please do not wake' +
          (userNamedList.length > 1 ? ' them' : '') +
          '.';
    ...
    if (
      ...
      (await userOptValue(guild, newUserID, 'voice'))
    ) {
  ...
  var PromisedOptVal = new Promise((resolve, reject) => {
    ...
    guild.members.fetch(userID).then((user) => {
      CheckAndCreateUser(userID, user.user.username)
        .then(async (result) => {
          var OptInOrOut = null;
          if (babadata.testing != undefined) OptInOrOut = optIn;
          else OptInOrOut = optOut;

          OptInOrOut(user, val)
            .then((result) => {
              guild.channels
                .fetch(babadata.botchan)
                .then((channel) => {
                  channel.send(
                    '<@' +
                      userID +
                      '> would you like to opt in for baba voice activity data analysis?\n' +
    ...
    if (global.dbAccess[1] && global.dbAccess[0]) {
      if (testingOveride && babadata.testing !== undefined) {
        ...
        IncrementCounters()
          ...
          .catch((err) => {
            resolve('Friday Counter Failed to Update (Testing Overide): ' + err);
          });
      }
      ...
      if (babadata.testing === undefined) {
        ...
        IncrementCounters()
          ...
          .catch((err) => {
            resolve('Friday Counter Failed to Update: ' + err);
          });
      }
    }
...
function FridayCounterIncrement() {
  var PromisedFridayCounter = new Promise((resolve, reject) => {
    ...
    var qureyStart =
      'INSERT INTO layersdeep (FridayUID,LoopsOrDOW,LayersDeep,Count,HeadingLevel,Sender) VALUES ';
    var qureyEnd =
      'AS newDeepLayers ON DUPLICATE KEY UPDATE layersdeep.Count = layersdeep.Count + newDeepLayers.Count;';
    var queryMiddle = '';
...
function FridayMessagesUpdate() {
  var PromisedFridayMessages = new Promise((resolve, reject) => {
    ...
    var qureyStart2 =
      'INSERT INTO myitisfriday (Sender,TimeStamp,Message,Condensed,Seed,FileVersion) VALUES ';
    var queryMiddle2 = '';
...
function LoadEmojiCache() {
  var PromisedEmoji = new Promise((resolve, reject) => {
    var emojiurl =
      'https://raw.githubusercontent.com/chalda-pnuzig/emojis.json/refs/heads/master/src/list.with.modifiers.json';
    ...
    callSQLQuery(query)
      .then((result) => {
        ...
        for (var i = 0; i < result.length; i++) {
          ...
          if (resj.StartDate != null) resj.StartDate.setFullYear(getD1().getFullYear());

          // if enddate != null set year to this year
          if (resj.EndDate != null) resj.EndDate.setFullYear(getD1().getFullYear());

          // if startdate == null set to earliest date
          if (resj.StartDate == null) resj.StartDate = new Date(0);

          // if enddate == null set to latest date
          if (resj.EndDate == null) resj.EndDate = new Date(8640000000000000);
          ...
          if (resj.IgnoredPhrases != null) resj.IgnoredPhrases = resj.IgnoredPhrases.split(',');
          else resj.IgnoredPhrases = [];
    ...
    callSQLQuery(query)
      .then((result) => {
        ...
        for (var i = 0; i < result.length; i++) {
          ...
          var files = null;
          if (res.Files != null && res.Files.length > 0) files = res.Files.split(',');
  ...
  var PromisedFriday = new Promise((resolve, reject) => {
    ...
    FridayAsync()
      .then(() => {
        ...
        var changes = false;
        if (newdowcache.length != tempdowcache.length) changes = true;
        else {
        ...
        if (!changes) {
          ...
          if (newdowcontrol.length != tempdowcontrol.length) changes = true;
          else {
        ...
        if (changes) {
          ...
          fs.readdir(babadata.datalocation + 'FridayCache', (err, files) => {
            ...
            fs.writeFileSync(
              babadata.datalocation + 'FridayCache/FridayLoops' + fcacheitems + '.json',
              data
            );
          });
          ...
          fs.readdir(babadata.datalocation + 'FridayCache', (err, files) => {
            ...
            fs.writeFileSync(
              babadata.datalocation + 'FridayCache/DOWcache' + dcacheitems + '.json',
              data
            );
          });
          ...
          fs.readdir(babadata.datalocation + 'FridayCache', (err, files) => {
            ...
            fs.writeFileSync(
              babadata.datalocation + 'FridayCache/DOWcontrol' + dcontrolitems + '.json',
              data
            );
          });
          ...
          if (babadata.testing === undefined) {
            ...
            con.query(
              `INSERT INTO timegates (VersionNumber, DateTime) VALUES ("${items}", "${newboy.DateTime.toISOString().slice(0, 19).replace('T', ' ')}")`,
              function (err, result) {
                if (err) {
...
function LoadFridayCache() {
  var PromisedFriday = new Promise((resolve, reject) => {
    var query =
      'SELECT * FROM dowfunny left join fridaytimegates on dowfunny.UID = fridaytimegates.fUID';
    ...
    callSQLQuery(query)
      .then((result) => {
        ...
        for (var i = 0; i < fridLoops.length; i++) {
          ...
          insertCount =
            gWeight == 1 ? fridLoops[i].weight : Math.floor((1 / gWeight) * fridLoops[i].weight);

          for (var j = 0; j < insertCount; j++)
            replacements[fridLoops[i].group].push({
              text: fridLoops[i].text,
              UID: fridLoops[i].UID,
            });
        }
...
function LoadHolidaysCache() {
  var PromisedHolidays = new Promise((resolve, reject) => {
    var query =
      'SELECT * FROM event left join alteventnames on event.EventID = alteventnames.EventID';
...
function groupEmojiByTones(emojiList) {
  ...
  for (var i = 0; i < list.length; i++) {
    ...
    if (eName.includes('skin tone')) {
      ...
      if (parent != null) parent.emojis.push(emoji.emoji);
      else {
        if (enameSplit != '') {
  ...
  return new Promise((resolve, reject) => {
    ...
    for (var i = 0; i < hurrInfo.length; i++) {
      if (hurrInfo[i].Updated) {
        ...
        con.query(
          `Insert into hurricane (id, name, number, type, category, imageURL, XMLUrl, Year, lastupdated) VALUES ("${id}", "${name}", "${number}", "${type}", "${category}", "${imgURL}", "${xmlURL}", "${year}", "${lastUpdated}") ON DUPLICATE KEY UPDATE name = "${name}", type = "${type}", category = "${category}", lastupdated = "${lastUpdated}"`,
          function (err, result) {
            if (err) {
              if (validErrorCodes(err.code)) {
                EnterDisabledMode(err);
...
async function getHurricaneInfo() {
  return new Promise((resolve, reject) => {
    ...
    saveUpdatedHurrInfo().then(() => {
      con.query('Select * from hurricane', function (err, result) {
        var opts = [];
        if (err) {
          if (validErrorCodes(err.code)) {
            EnterDisabledMode(err);
        ...
        for (var i = 0; i < result.length; i++) {
          ...
          if (res.Year != getD1().getFullYear()) continue;
