  ...
  var morshuPromise = new Promise((resolve, reject) => {
    fetch('https://morshu.yoinks.org/morsh', {
      ...
      .then((data) => {
        ...
        if (mode == 'audio') {
          var newFile = new Discord.AttachmentBuilder(nodeBuffer, {
            name: 'Morshu.mp3',
            description: text,
          });
  ...
  var channelGetPromise = new Promise((resolve, reject) => {
    ...
    global.Bot.guilds
      ...
      .then((guild) => {
        ...
        if (channel != null) resolve(channel.name);
        else resolve('Channel not found');
      })
      .catch((error) => {
        console.error(error);
        resolve('Guild not found');
  ...
  var roleGetPromise = new Promise((resolve, reject) => {
    ...
    global.Bot.guilds
      ...
      .then((guild) => {
        ...
        if (role != null) resolve(role.name);
        else resolve('Role not found');
      })
      .catch((error) => {
        console.error(error);
        resolve('Guild not found');
...
function parseDiscordSpecial(text) {
  ...
  return matches
    .map((match) => {
      const [fullMatch, , channelId, roleId, userId, animatedFlag, emojiName, emojiId, plainText] =
        match;
